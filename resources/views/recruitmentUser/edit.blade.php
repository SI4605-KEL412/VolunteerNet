...
<head>
    ...
    <title>Edit Recruitment User</title>
    ...
    <script>
        tailwind.config = {
            ...
            plugins: [
                // tailwindcss/forms tidak bisa dipanggil langsung di browser, jadi abaikan jika error
            ],
        }
    </script>
    <style>
    ...
    <div class="lg:ml-64 min-h-screen">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <h1 class="text-3xl font-bold text-white mb-8 flex items-center">
                <i class="bi bi-pencil-square text-blue-400 mr-3 text-4xl"></i> Edit Pendaftaran Event
            </h1>
            @if(session('success'))
            ...
            @if ($errors->any())
                <div class="bg-red-800/40 border border-red-600/50 text-red-300 px-5 py-4 rounded-xl mb-6 shadow-lg animate-fade-in" role="alert">
            ...
            <form action="{{ route('recruitmentUser.update', $recruitment->recruitment_id) }}" method="POST" class="bg-white/10 backdrop-blur-md shadow-2xl rounded-2xl border border-slate-700/50 p-6 md:p-8 animate-fade-in">
                @csrf
                @method('PUT')
                <div class="mb-5">
                    <label for="motivation" class="block text-sm font-semibold text-slate-300 mb-1.5">Motivasi Mengikuti Event Ini</label>
                    <textarea name="motivation" id="motivation" class="form-input mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400 text-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg shadow-sm text-base px-4 py-3" rows="4" maxlength="1000" required>{{ old('motivation', $recruitment->motivation) }}</textarea>
                </div>
                <div class="flex justify-between mt-6">
                    <a href="{{ route('recruitmentUser.index') }}" class="inline-flex items-center px-6 py-2 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-700 hover:text-white transition-colors duration-200 transform hover:scale-105">
                        <i class="bi bi-arrow-left mr-2"></i> Kembali ke Daftar
                    </a>
                    <button type="submit" class="inline-flex items-center px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200 transform hover:scale-105 font-semibold shadow-md">
