@php
    // use Illuminate\Support\Str; // Tidak digunakan di sini
@endphp
...
<head>
    ...
    <title>Generate Sertifikat dari Event</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-10">
                <h1 class="text-3xl md:text-4xl font-bold text-white mb-4 sm:mb-0 flex items-center">
                    <i class="bi bi-award text-amber-400 mr-3 text-4xl"></i> Event & Generate Sertifikat
                </h1>
                <a href="{{ route('certifications.index') }}" class="inline-flex items-center px-6 py-2.5 border border-slate-600 text-slate-300 rounded-xl hover:bg-slate-700 hover:text-white transition-colors duration-200 transform hover:scale-105 font-medium text-sm shadow-md">
                    <i class="bi bi-arrow-left-circle-fill mr-2"></i>Kembali ke Sertifikat Saya
                </a>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-xl shadow-2xl rounded-2xl border border-slate-700/50 animate-fade-in overflow-hidden">
                @if($events->isEmpty())
                    <div class="text-center py-16">
                        ...
                        <p class="text-2xl font-semibold text-white mb-2">Tidak Ada Event</p>
                        <p class="text-slate-400">Saat ini tidak ada event yang tersedia untuk generate sertifikat.</p>
                    </div>
                @else
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            <thead class="bg-slate-800/50">
                                <tr>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Judul Event</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Tanggal Event</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Status Sertifikat</th>
                                    <th scope="col" class="px-6 py-4 text-center text-xs font-semibold text-slate-300 uppercase tracking-wider">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-700/60">
                                @php
                                    // Asumsi $certifications adalah collection dari sertifikat milik user saat ini,
                                    // di-pass dari controller dan di-key berdasarkan event_id untuk pencarian cepat.
                                    // Jika tidak di-key, firstWhere akan bekerja tapi mungkin kurang optimal untuk banyak data.
                                    // $userCertificationsByEventId = $certifications->keyBy('event_id');
                                @endphp
                                @foreach($events as $event)
                                    @php
                                        // $cert = $userCertificationsByEventId->get($event->event_id);
                                        // Menggunakan firstWhere sesuai kode asli Anda jika $certifications bukan keyed collection
                                        $cert = $certifications->firstWhere('event_id', $event->event_id);
                                    @endphp
                                    <tr class="hover:bg-slate-700/40 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-100">{{ $event->title }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{{ \Carbon\Carbon::parse($event->event_date ?? $event->start_date)->isoFormat('LL') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            @if ($cert)
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-green-700/40 text-green-300 ring-1 ring-inset ring-green-600/50">
                                                    <i class="bi bi-check-circle-fill mr-1.5"></i>Sudah Digenerate
                                                </span>
                                            @else
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-slate-600/50 text-slate-300 ring-1 ring-inset ring-slate-500/50">
                                                    <i class="bi bi-hourglass-split mr-1.5"></i>Belum Digenerate
                                                </span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                            @if ($cert)
                                                {{-- Tombol untuk melihat sertifikat yang sudah ada (jika ada route-nya) --}}
                                                <a href="{{-- route('certifications.show', $cert->cert_id) --}}" class="inline-flex items-center bg-sky-600/80 hover:bg-sky-700/90 text-white px-4 py-2 rounded-lg text-xs font-semibold transition-colors shadow-sm hover:shadow-md" title="Lihat Sertifikat">
                                                    <i class="bi bi-eye-fill mr-1.5"></i>Lihat
                                                </a>
                                            @else
                                                <a href="{{ route('certifications.generate', $event->event_id) }}" class="inline-flex items-center bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-4 py-2 rounded-lg text-xs font-semibold transition-all shadow-md hover:shadow-lg transform hover:scale-105">
                                                    <i class="bi bi-patch-plus-fill mr-1.5"></i>Generate Sertifikat
                                                </a>
                                            @endif
                                        </td>
                                    </tr>
            ...
            @if ($events instanceof \Illuminate\Pagination\AbstractPaginator && $events->hasPages())
                <div class="mt-8">
                    {{ $events->links() }} {{-- Sesuaikan view pagination jika perlu --}}
                </div>
            @endif
