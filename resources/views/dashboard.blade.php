...
<body class="bg-gray-100">
    ...
    <main class="flex-1 p-10">
      <div id="volunteerDashboard">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <!-- Additional Event Cards Go Here -->
        </div>
      </div>
      ...
      <div id="eoDashboard" class="hidden">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
            <!-- Event Card for EO -->
            <div class="bg-white rounded-xl shadow p-6 flex flex-col justify-between">
              <img src="https://static.promediateknologi.id/crop/0x0:0x0/0x0/webp/photo/p2/72/2025/03/26/WhatsApp-Image-2025-03-26-at-175907-3801901328.jpeg" alt="Event Image" class="w-full h-40 object-cover mb-4">
              <div>
                ...
                <p class="text-sm text-gray-600 mb-4">🗓️ 27 May 2025</p>
              </div>
              <div class="mt-auto flex gap-2">
                <button class="w-full bg-gray-200 text-[#0B2447] font-semibold py-2 px-4 rounded hover:bg-gray-300 transition">Detail</button>
                <button class="w-full bg-[#19376D] text-white py-2 px-4 rounded hover:bg-[#576CBC] transition">Manage</button>
              </div>
            </div>
            <!-- Additional EO Event Cards Go Here -->
          </div>
        </div>
      </div>
    </main>
  ...
  <script>
    ...
    function toggleMode(input) {
      if (input.checked) {
        document.getElementById('volunteerMenu').classList.add('hidden');
        document.getElementById('eoMenu').classList.remove('hidden');
        document.getElementById('volunteerDashboard').classList.add('hidden');
        document.getElementById('eoDashboard').classList.remove('hidden');
      } else {
        document.getElementById('volunteerMenu').classList.remove('hidden');
        document.getElementById('eoMenu').classList.add('hidden');
        document.getElementById('volunteerDashboard').classList.remove('hidden');
        document.getElementById('eoDashboard').classList.add('hidden');
      }
    }
  </script>
</body>
