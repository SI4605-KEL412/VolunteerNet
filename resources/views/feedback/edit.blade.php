...
<head>
    ...
    <style>
        ...
        input[type="datetime-local"]::-webkit-calendar-picker-indicator {
            filter: invert(0.8) brightness(1.2); /* Coba sesuaikan nilai ini jika perlu */
        }
    </style>
</head>
    ...
    <div class="lg:ml-64 min-h-screen">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-3xl md:text-4xl font-bold text-white">Edit Feedback</h1>
            ...
            <div class="bg-white/10 backdrop-blur-md shadow-2xl rounded-2xl border border-slate-700/50 p-6 md:p-10 animate-fade-in">
                <form action="{{ route('feedback.update', $feedback->feedback_id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="event_id" id="event_id" class="mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400 text-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg shadow-sm text-sm px-4 py-3" required>
                            ...
                            @forelse($events as $event)
                                <option value="{{ $event->event_id }}" {{ old('event_id', $feedback->event_id) == $event->event_id ? 'selected' : '' }} class="text-slate-200 bg-slate-800">{{ $event->title }}</option>
                            @empty
                    ...
                    <div>
                        <label for="rating" class="block text-sm font-semibold text-slate-300 mb-1.5">Rating</label>
                    ...
                    <div>
                        ...
                        <textarea class="mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400/80 text-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg shadow-sm text-sm px-4 py-3 min-h-[120px] resize-y" name="comments" id="comments" rows="4" maxlength="500" placeholder="Tulis komentar kamu di sini (opsional)...">{{ old('comments', $feedback->comments) }}</textarea>
                    </div>

                    <div>
                        <label for="date_given" class="block text-sm font-semibold text-slate-300 mb-1.5">Tanggal Diberikan</label>
                        <input type="datetime-local" class="mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400 text-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg shadow-sm text-sm px-4 py-3" name="date_given" id="date_given" value="{{ old('date_given', \Carbon\Carbon::parse($feedback->date_given)->format('Y-m-d\TH:i')) }}" required>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center pt-6 space-y-4 sm:space-y-0 sm:space-x-4 border-t border-slate-700/50">
                        ...
                        <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl hover:from-blue-600 hover:to-blue-700 transition-all duration-200 transform hover:scale-105 shadow-xl font-semibold text-sm">
                            <i class="bi bi-check-circle-fill mr-2"></i> Update Feedback
                        </button>
                    </div>
                </form>
            </div>
