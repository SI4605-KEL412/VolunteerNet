...
<head>
    ...
    <title>Buat Topik Forum Baru</title>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-3xl md:text-4xl font-bold text-white flex items-center">
                    <i class="bi bi-plus-square-dotted text-purple-400 mr-3"></i> Buat Topik Forum Baru
                </h1>
                 <a href="{{ route('forums.index') }}" class="inline-flex items-center px-6 py-2 border border-slate-600 text-slate-300 rounded-lg hover:bg-slate-700 hover:text-white transition-colors duration-200 transform hover:scale-105 text-sm">
            ...
            <div class="bg-white/10 backdrop-blur-md shadow-2xl rounded-2xl border border-slate-700/50 p-6 md:p-10 animate-fade-in">
                <form action="{{ route('forums.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" class="form-input mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400/80 text-slate-100 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 rounded-lg shadow-sm text-base px-4 py-3" value="{{ old('title') }}" required placeholder="Masukkan judul yang menarik...">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="8" class="form-textarea mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400/80 text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 rounded-lg shadow-sm text-base px-4 py-3 min-h-[200px] resize-y" required placeholder="Jelaskan topik diskusi Anda secara detail...">{{ old('content') }}</textarea>
                         @error('content')
                            <p class="mt-2 text-xs text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    {{-- <div>
                        <label for="category_id" class="block text-sm font-semibold text-slate-300 mb-1.5">Kategori (Opsional)</label>
                        <select name="category_id" id="category_id" class="form-select mt-1 block w-full bg-slate-800/70 border-slate-700 text-slate-200 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 rounded-lg shadow-sm text-base px-4 py-3">
                            ...
                            @foreach($categories as $category) {{-- Asumsi $categories di-pass dari controller --}}
                                {{-- <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }} class="bg-slate-800">{{ $category->name }}</option> --}}
                            {{-- @endforeach
                        </select>
                         @error('category_id')
                            <p class="mt-2 text-xs text-red-400">{{ $message }}</p>
                    ...
                    <div class="flex flex-col sm:flex-row justify-end items-center pt-6 space-y-4 sm:space-y-0 sm:space-x-4 border-t border-slate-700/50">
                        <a href="{{ route('forums.index') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3 border border-slate-600 text-slate-300 rounded-xl hover:bg-slate-700 hover:text-white transition-colors duration-200 transform hover:scale-105 font-semibold text-sm shadow-md">
                            <i class="bi bi-x-lg mr-2"></i> Batal
                        </a>
                        <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-xl hover:from-purple-600 hover:to-indigo-700 transition-all duration-200 transform hover:scale-105 shadow-xl font-semibold text-sm">
                            <i class="bi bi-send-plus-fill mr-2"></i> Simpan Topik
                        </button>
                    </div>
                </form>
            </div>
