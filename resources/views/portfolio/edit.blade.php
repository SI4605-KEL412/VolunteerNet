...
<head>
    ...
    <title>Edit Entri Portofolio: {{ $portfolio->title }} - User Dashboard</title>
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-8 animate-fade-in">
                <h1 class="text-3xl md:text-4xl font-bold text-white mb-4 sm:mb-0 flex items-center">
                    <i class="bi bi-pencil-square text-yellow-400 mr-3 text-4xl"></i> Edit Entri Portofolio
                </h1>
            ...
            <div class="bg-white/10 backdrop-blur-xl shadow-2xl rounded-2xl border border-slate-700/50 p-6 md:p-10 animate-fade-in">
                ...
                <form action="{{ route('portfolio.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" class="form-input mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400/80 text-slate-100 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 rounded-lg shadow-sm text-base px-4 py-3 @error('title') border-red-500/70 focus:border-red-500 focus:ring-red-500 @enderror" value="{{ old('title', $portfolio->title) }}" required placeholder="Contoh: Partisipasi dalam Bakti Sosial XYZ">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" class="form-textarea mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400/70 text-slate-100 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 rounded-lg shadow-sm text-sm px-4 py-3 min-h-[150px] resize-y @error('description') border-red-500/70 focus:border-red-500 focus:ring-red-500 @enderror" rows="5" placeholder="Jelaskan kontribusi dan pengalaman Anda dalam kegiatan ini...">{{ old('description', $portfolio->description) }}</textarea>
                        @error('description')
                            <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" name="date" id="date" class="form-input mt-1 block w-full bg-slate-800/70 border-slate-700 text-slate-200 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 rounded-lg shadow-sm text-sm px-4 py-3 @error('date') border-red-500/70 focus:border-red-500 focus:ring-red-500 @enderror" value="{{ old('date', $portfolio->date ? \Carbon\Carbon::parse($portfolio->date)->format('Y-m-d') : '') }}">
                            @error('date')
                                <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="text" name="location" id="location" class="form-input mt-1 block w-full bg-slate-800/70 border-slate-700 placeholder-slate-400/80 text-slate-100 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 rounded-lg shadow-sm text-sm px-4 py-3 @error('location') border-red-500/70 focus:border-red-500 focus:ring-red-500 @enderror" value="{{ old('location', $portfolio->location) }}" placeholder="Contoh: Desa Sukamaju, Online">
                            @error('location')
                                <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        <label for="file" class="block text-sm font-semibold text-slate-300 mb-1.5">Upload File Baru (Opsional)</label>
                        ...
                        <small class="mt-1.5 text-xs text-slate-500 block">Kosongkan jika tidak ingin mengganti file. Format: PDF, JPG, PNG. Max 2MB.</small>
                        @error('file')
                            <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p>
                        @enderror
                        ...
                        @if ($portfolio->file_path)
                            <div class="mt-3">
                                <p class="text-xs text-slate-400 mb-1">File saat ini:</p>
                                <a href="{{ asset('storage/portfolio_files/' . $portfolio->file_path) }}" target="_blank" class="inline-flex items-center text-sm font-medium text-sky-400 hover:text-sky-300 transition-colors group">
                                    <i class="bi bi-paperclip mr-1.5 text-base transform group-hover:rotate-[-12deg] transition-transform"></i> 
                                    {{-- Menampilkan nama file jika memungkinkan, atau teks generik --}}
                                    {{ Str::afterLast($portfolio->file_path, '/') ?: 'Lihat File Lampiran' }}
                                </a>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row justify-end items-center pt-6 space-y-4 sm:space-y-0 sm:space-x-4 border-t border-slate-700/50">
                        ...
                        <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center px-7 py-2.5 bg-gradient-to-r from-yellow-500 to-amber-600 text-slate-900 rounded-xl hover:from-yellow-600 hover:to-amber-700 transition-all duration-200 transform hover:scale-105 shadow-xl font-semibold text-sm">
                            <i class="bi bi-check-circle-fill mr-2"></i> Update Portofolio
                        </button>
                    </div>
                </form>
            </div>
