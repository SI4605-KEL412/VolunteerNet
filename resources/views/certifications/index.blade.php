        ...
        <nav class="p-4 space-y-2">
            ...
            <div class="pt-4 space-y-3 border-t border-slate-700">
                ...
                <a href="{{ route('certifications.index') }}" class="action-btn {{ request()->routeIs('certifications.index') || request()->routeIs('certifications.events') ? 'ring-2 ring-amber-400 bg-gradient-to-r from-amber-600 to-amber-700' : 'bg-gradient-to-r from-amber-500 to-amber-600' }} block w-full text-center px-4 py-3 text-white rounded-lg hover:from-amber-600 hover:to-amber-700 transition-all duration-200 transform hover:scale-105 shadow-lg">
                    <i class="bi bi-award-fill mr-2"></i> {{-- Mengubah ikon --}}
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-10">
                <h1 class="text-3xl md:text-4xl font-bold text-white mb-4 sm:mb-0 flex items-center">
                    <i class="bi bi-patch-check-fill text-amber-400 mr-3 text-4xl"></i> Daftar Sertifikat Saya
                </h1>
             ...
             <div class="mb-8 animate-fade-in">
                <a href="{{ route('certifications.events') }}" class="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-xl hover:from-sky-600 hover:to-blue-700 transition-all duration-200 transform hover:scale-105 shadow-xl font-semibold text-base">
                    <i class="bi bi-calendar2-event-fill mr-2.5"></i> Lihat Event & Generate Sertifikat Baru
                </a>
            </div>
            ...
            <div class="bg-white/10 backdrop-blur-xl shadow-2xl rounded-2xl border border-slate-700/50 animate-fade-in overflow-hidden">
                @if($certifications->isEmpty())
                    <div class="text-center py-16">
                        <i class="bi bi-journal-x text-6xl text-slate-500 mb-6"></i>
                        <p class="text-2xl font-semibold text-white mb-2">Belum Ada Sertifikat</p>
                        <p class="text-slate-400">Anda belum memiliki sertifikat yang diterbitkan.</p>
                    </div>
                @else
                    <div class="overflow-x-auto">
                        <table class="min-w-full">
                            <thead class="bg-slate-800/50">
                                <tr>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Judul Sertifikat</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Event</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Tanggal Terbit</th>
                            ...
                            <tbody class="divide-y divide-slate-700/60">
                                ...
                                <tr class="hover:bg-slate-700/40 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-100">{{ $cert->title }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{{ $cert->event->title ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{{ \Carbon\Carbon::parse($cert->issued_date)->isoFormat('LL') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                        <form action="{{ route('certifications.destroy', $cert->cert_id) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus sertifikat ini?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center bg-rose-600/80 hover:bg-rose-700/90 text-white px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-colors shadow-sm hover:shadow-md">
                                                <i class="bi bi-trash3-fill mr-1.5"></i>Hapus
                                            </button>
                                        </form>
                                        {{-- Tambahkan tombol lihat/download jika ada --}}
                                        {{-- <a href="{{ route('certifications.show', $cert->cert_id) }}" class="ml-2 inline-flex items-center bg-sky-600/80 hover:bg-sky-700/90 text-white px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-colors shadow-sm hover:shadow-md">
             ...
             @if ($certifications instanceof \Illuminate\Pagination\AbstractPaginator && $certifications->hasPages())
                <div class="mt-8">
                    {{ $certifications->links() }} {{-- Sesuaikan view pagination jika perlu --}}
                </div>
            @endif
        </div>
    </div>
